...
func TestinterpolationSearch(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != tt.expect {
				t.Errorf("Test failed: %s - Expected %d, got %d", tt.name, tt.expect, result)
			} else {
				t.Logf("Test passed: %s - Got expected index %d", tt.name, result)
			}
		})
	}

	// TODO: Consider testing concurrent scenarios if applicable
	// Add further coverage for edge cases if logic is extended.
}
